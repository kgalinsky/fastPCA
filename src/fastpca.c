...
uint8_t *
read_mask (size_t n, const char *filename);
...
parse_args (int argc, char **argv)
{
  ...
  if (optind == argc - 1)
    ...
    print_usage ("fastpca: No input file specified");
  else
    print_usage ("fastpca: Too many arguments");

  if (K >= L)
    print_usage ("fastpca: K >= L");
