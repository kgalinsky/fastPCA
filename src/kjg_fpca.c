...
void kjg_fpca_XTXG (
    ...
    for (i = 0; i < X->m; i += KJG_FPCA_ROWS) {
        ...
        gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 1, &Xmat.matrix, G1, 0,
                &Hmat.matrix);
...
void kjg_fpca_XG (
    ...
    for (i = 0; i < X->m; i += KJG_FPCA_ROWS) {
        ...
        Xmat = gsl_matrix_view_array(Y, r, X->n);
        Hmat = gsl_matrix_submatrix(H, i, 0, r, H->size2);
        gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 1, &Xmat.matrix, G, 0,
                &Hmat.matrix);
    }
...
void kjg_fpca_XTH (
    ...
    for (i = 0; i < X->m; i += KJG_FPCA_ROWS) {
        r = kjg_geno_get_normalized_rows(x, Y, X, M, i, KJG_FPCA_ROWS);
        Xmat = gsl_matrix_view_array(Y, r, X->n);
        gsl_matrix_const_view Hmat = gsl_matrix_const_submatrix(H, i, 0, r,
                H->size2);
        gsl_blas_dgemm(CblasTrans, CblasNoTrans, 1, &Xmat.matrix, &Hmat.matrix,
                1, G);
    }
